class RefundSystem {
  ...
  setupEventListeners() {
    const searchTransactionBtn = document.getElementById(
    ...
    const closeRefundReceiptModal = document.getElementById(
      "closeRefundReceiptModal"
    );
    if (closeRefundReceiptModal) {
      closeRefundReceiptModal.addEventListener("click", () =>
    ...
    refundResults.innerHTML = `
            ...
            <div class="transaction-results-grid">
                ${results
                  .map(
                    (transaction) => `
                    <div class="transaction-result-card" data-transaction-id="${
                      transaction.id
                    }">
                        ...
                        <div class="transaction-result-body">
                            <p><i class="fas fa-calendar"></i> ${this.formatDate(
                              transaction.date
                            )}</p>
                            <p><i class="fas fa-shopping-cart"></i> ${
                              transaction.items.length
                            } item(s)</p>
                            <p><i class="fas fa-money-bill-wave"></i> ${this.formatCurrency(
                              transaction.total
                            )}</p>
                            <p><i class="fas fa-credit-card"></i> ${this.getPaymentMethodText(
                              transaction.paymentMethod
                            )}</p>
                        </div>
                        <div class="transaction-result-footer">
                            <button class="select-transaction-btn" onclick="refundSystem.selectTransaction('${
                              transaction.id
                            }')">
  ...
  selectTransaction(transactionId) {
    ...
    if (this.selectedTransaction) {
      ...
      this.showMessage(
        "Transaksi dipilih. Pilih item yang akan di-refund.",
        "success"
      );
    }
  }
                        ...
                        <div class="refund-item-controls">
                            <div class="refund-checkbox-container">
                                <label class="refund-checkbox-label">
                                    <input type="checkbox" class="refund-checkbox" data-item-id="${
                                      item.id
                                    }" data-item-price="${
                      item.price * item.quantity
                    }">
                            ...
                            <div class="refund-reason-container">
                                ...
                                <select class="refund-reason" data-item-id="${
                                  item.id
                                }">
  ...
  updateRefundSummary() {
    ...
    const refundSummaryContent = document.getElementById(
      "refundSummaryContent"
    );
    ...
    if (refundSummary && refundSummaryContent) {
      ...
      refundSummaryContent.innerHTML = `
                ...
                <div class="refund-summary-item">
                    ...
                    <span class="refund-total">${this.formatCurrency(
                      this.refundTotal
                    )}</span>
                </div>
            `;
  ...
  showRefundConfirmation() {
    ...
    const refundItems = Array.from(checkedItems).map((checkbox) => {
      ...
      const reason = document.querySelector(
        `[data-item-id="${itemId}"].refund-reason`
      ).value;
      const originalItem = this.selectedTransaction.items.find(
        (item) => item.id === itemId
      );
  ...
  showRefundReceipt(refund) {
    ...
    content.innerHTML = `
            ...
            <div style="border-top: 1px dashed #ccc; border-bottom: 1px dashed #ccc; padding: 15px 0; margin: 15px 0;">
                ...
                <p><strong>ID Transaksi Asli:</strong> ${
                  refund.originalTransactionId
                }</p>
                <p><strong>Tanggal Refund:</strong> ${this.formatDate(
                  refund.date
                )}</p>
                    ...
                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px; padding: 5px 0; border-bottom: 1px dotted #ddd;">
                        <div>
                            ...
                            <div style="font-size: 0.9em; color: #666;">${
                              item.quantity
                            } x ${this.formatCurrency(item.price)}</div>
                            <div style="font-size: 0.8em; color: #888; font-style: italic;">Alasan: ${this.getRefundReasonText(
                              item.reason
                            )}</div>
                        </div>
                        <span style="font-weight: bold;">${this.formatCurrency(
                          item.refundAmount
                        )}</span>
                    </div>
                `
        ...
        (refund) => `
            <div class="recent-refund-card">
                <div class="refund-card-header">
                    <h5>${refund.id}</h5>
                    <span class="refund-amount">${this.formatCurrency(
                      refund.total
                    )}</span>
                </div>
                <div class="refund-card-body">
                    <p><i class="fas fa-receipt"></i> Transaksi: ${
                      refund.originalTransactionId
                    }</p>
                    <p><i class="fas fa-calendar"></i> ${this.formatDate(
                      refund.date
                ...
                <div class="refund-card-footer">
                    <button class="view-refund-btn" onclick="refundSystem.viewRefundDetails('${
                      refund.id
                    }')">
